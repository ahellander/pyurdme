...
class  dimerization(URDMEModel):
  ...
  def __init__(self, model_name="dimerization"):
    ...
    facet_function = dolfin.MeshFunction("size_t",self.mesh.mesh)
    file_in >> facet_function

    file_in = dolfin.File("meshes/plane_physical_region.xml")
    physical_region = dolfin.MeshFunction("size_t",self.mesh.mesh)
    file_in >> physical_region

    #print self.mesh.mesh.num_facets()
    ...
    self.scatter(A,subdomain=1)
    self.scatter(B,subdomain=1)
    self.scatter(C,subdomain=1)

    # Time span of the simulation
    self.timespan(numpy.arange(0,25,0.1))
