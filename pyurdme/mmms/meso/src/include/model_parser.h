...
void print_model(vector <species>& specs,vector <association>& associations,vector <dissociation>& dissociations,vector <birth>& births,simulation *sim){
    ...
    for(int i=0;i<(int)(specs.size());i++){
        ...
        if(specs[i].meso_micro==0){
            printf("Species is mesoscopic.\n");
        }
        else if(specs[i].meso_micro==1){
            printf("Species is microscopic\n");
        }
        else{
            printf("WARNING: Species not set to either microscopic or mesoscopic.\n");
        }
    }
    printf("********************\n");
    ...
    for(int i=0;i<(int)(associations.size());i++){
        ...
        if((int)(associations[i].products.size())==1){
            printf("%s, k=%.5g, kmeso=%.5g, r=%.5g\n",specs[associations[i].products[0]].name,associations[i].k,associations[i].kmeso,associations[i].r);
        }
        else if((int)(associations[i].products.size())==2){
            printf("S%d+S%d, k=%.5g, kmeso=%.5g, r=%.5g\n",associations[i].products[0],associations[i].products[1],associations[i].k,associations[i].kmeso,associations[i].r);
        }
    }
    printf("--------------------\n");
    printf("Unimolecular:\n");
    ...
    for(int i=0;i<(int)(dissociations.size());i++){
        ...
        if((int)(dissociations[i].products.size())==1){
            printf("%s, k=%.5g, kmeso=%.5g\n",specs[dissociations[i].products[0]].name,dissociations[i].k,dissociations[i].kmeso);
        }
        else if((int)(dissociations[i].products.size())==2){
            printf("%s+%s, k=%.5g, kmeso=%.5g\n",specs[dissociations[i].products[0]].name,specs[dissociations[i].products[1]].name,dissociations[i].k,dissociations[i].kmeso);
        }
        else if((int)(dissociations[i].products.size())==0){
            printf("0, k=%.5g, kmeso=%.5g\n",dissociations[i].k,dissociations[i].kmeso);
        }
    }
    printf("--------------------\n");
...
//    printf("reading reaction: %s, %s\n",r,p);
    ...
    if(num_reactants==2){
        ...
        /* Get mesoscopic rate. */
//        double sigma_temp = specs[temp_reac.reactant1].sigma+specs[temp_reac.reactant2].sigma;
//        double D_temp = specs[temp_reac.reactant1].D+specs[temp_reac.reactant2].D;
//        double krmeso_temp;
//        double r_temp = 0.0;
//        double L = 1e-6;
//        int N = pow(200,3);
//        int dimension = 3;
//        if(sim->num_voxels<0 || sim->dimension<0 || sim->volume<0){
//            printf("You have to set the dimension, boundary, and number of voxels before reactions.\nThis information is required to convert microscopic rates to mesoscopic rates.");
//            exit(EXIT_FAILURE);
//        }
//        non_local_k_r(pow(sim->volume,1.0/(double)(sim->dimension)),pow(sim->num_voxels,sim->dimension),sigma_temp,D_temp,temp_reac.k,&krmeso_temp,&r_temp,sim->dimension);
//        temp_reac.kmeso = krmeso_temp;
        temp_reac.r = 0.0;//r_temp;
        /* .................... */
...
void add_species(char *sp,vector <species>& specs,vector <parameter>& params){
    ...
    str = strtok(NULL," ");
    if(str==NULL){
        printf("Error when reading data for species: %s (5)\n",sp);
        exit(EXIT_FAILURE);
    }

    if(strcmp(str,"MESO")==0){
        temp.meso_micro = 0;
    }
    else if(strcmp(str,"MICRO")==0){
        temp.meso_micro = 1;
    }
    else{
        printf("Error: %s (5)\nA species has to be set to either MICRO or MESO.\n",sp);
        exit(EXIT_FAILURE);
    }
...
void parse_voxels(char *input,simulation *sim,vector <parameter>& params){
    int status;
    char err_msg[MAX_CHARACTERS];
    sprintf(err_msg,"Error reading number of voxels: ");
    char *str;
    str = fetch_next(input,err_msg,true);

    sim->num_voxels = (int)(parse_parameter(str,params,&status));
    if(sim->num_voxels<=0){
        printf("Error: N=%d\nNumber of voxels has to be a positive number.\n",sim->num_voxels);
        exit(EXIT_FAILURE);
    }
}
...
void parse_model(char *filename,simulation *sim,vector <species>& specs,vector <association>& assocs,vector <dissociation>& dissocs,vector <birth>& births,vector <parameter>& parameters){
    ...
    const char *keywords[] = {"NAME","SPECIES","PARAMETER","REACTION","OUTPUT","NTRAJ","NCORES","T","NINT","MESH"};
...
//                printf("%s\n",input);
                sim->num_intervals = atoi(input);//strtol(input,&ptr,10);
                // printf("hej");
...
//                }
//            }
//            else if(strcmp(keywords[keyword_loc],"VOXELS")==0){
//                parse_voxels(input,sim,parameters);
////                printf("Read number of voxels. Result: N=%d\n",sim->num_voxels);
////                add_boundary(input,sim,parameters);
////                sim->volume = 1.0;
////                for(int i=0;i<sim->dimension;i++){
////                    sim->volume *= sim->boundary[2*i+1]-sim->boundary[2*i];
////                }
//            }
            else if(strcmp(keywords[keyword_loc],"MESH")==0){
                memset(sim->mesh,'\0',sizeof(sim->mesh));
                strcpy(sim->mesh,input);
            }
...
//            printf("LINE IS NOW: %s\n",line);
        ...
        /*Set mesoscopic dissociation rates. */
//        for(int i=0;i<(int)(dissocs.size());i++){
//            int rev = reversible(&(dissocs[i]),assocs);
//            if(rev>=0){
//                double sigma_temp = specs[assocs[rev].reactant1].sigma+specs[assocs[rev].reactant2].sigma;
//                double D_temp = specs[assocs[rev].reactant1].D+specs[assocs[rev].reactant2].D;
//                double r_temp;
//
//                if(sim->num_voxels<0 || sim->dimension<0 || sim->volume<0){
//                    printf("You have to set the dimension, boundary, and number of voxels.\n");
//                    exit(EXIT_FAILURE);
//                }
//                double L = pow(sim->volume,1.0/sim->dimension);
//                int N = pow(sim->num_voxels,sim->dimension);
////                int dimension = 3;
//                dissocs[i].kmeso = non_local_k_d(L,N,sigma_temp,D_temp,assocs[rev].kmeso,dissocs[i].k,&r_temp,sim->dimension);
//            }
//            else{
